...
def metropolis(spin):
    ...
    total_m = m / (N * N)         
    return spin, total_E, total_m   
    
# Define animation function
def animate(frame):
    global spin
    for i in range(steps):
        spin, total_E, total_m = metropolis(spin)
        E.append(total_E)
        M.append(total_m)
...
anim.save(f"Ising Model for {T} T, {F} frames, {N} grid.gif", writer = "pillow")
print(f"Animation was successfully saved as 'Ising Model for {T} T, {F} frames, {N} grid.gif' within the program directory.")
